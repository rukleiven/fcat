...
def test_actuator_model():
    ...
    initial_values = np.array([initial_value_elevator, initial_value_aileron, 0, initial_value_throttle])
    ...
    expect_elevon_r = control_input.elevon_right + (initial_value_elevon_right - control_input.elevon_right)*np.exp(-t/elevon_time_constant)
    expect_elevon_l = control_input.elevon_left + (initial_value_elevon_left - control_input.elevon_left)*np.exp(-t/elevon_time_constant)
    expect_motor = control_input.throttle + (initial_value_throttle - control_input.throttle)*np.exp(-t/motor_time_constant)
